...
6. Analyze the staged changes and draft a commit message in this exact format:
...
8. Stage the session files (by name — NEVER `git add .` or `git add -A`). If the session touched documentation (`README.md`, `CLAUDE.md`, `.claude/rules/<module>-module.md`), those are session files like any other and get staged too.
...
- Run the entire flow (log → message → stage → commit → status → push) in one continuous response. Do NOT pause for user input between steps unless a real blocker appears (hook failure, push rejection, ambiguous file ownership). The user invoked `/commit` once; finish it.
